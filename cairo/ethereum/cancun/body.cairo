...
func main{
    ...
    local chain_id: U64;
    local base_fee_per_gas: Uint;
    local blob_gas_used: Uint;
    local excess_blob_gas: U64;
    %{ body_inputs %}

    // TODO: once init merged
    // let (initial_args_commitment) = init_commitment(block, state, transactions_trie, receipts_trie, block_logs, block_hashes, gas_available, chain_id, base_fee_per_gas, excess_blob_gas);
    ...
    with state, transactions_trie, receipts_trie {
        let (blob_gas_used, gas_available, block_logs) = _apply_body_inner(
            ...
            block_number=block.value.header.value.number,
            block_gas_limit=block.value.header.value.gas_limit,
            block_time=block.value.header.value.timestamp,
    ...
    // let (post_args_commitment) = init_commitment(block, state, transactions_trie, receipts_trie, block_logs, block_hashes, gas_available, chain_id, base_fee_per_gas, excess_blob_gas);

    assert [output_ptr] = 'some_value';
    assert [output_ptr + 1] = 'some_value';
    assert [output_ptr + 2] = 'some_value';
    assert [output_ptr + 3] = 'some_value';
