...
func main{
    ...
    %{
        ...
        ids.node_store = gen_arg(Mapping[Hash32, InternalNode], program_input["node_store"])
        ids.address_preimages = gen_arg(Mapping[Hash32, Address], program_input["address_preimages"])
        ids.storage_key_preimages = gen_arg(Mapping[Hash32, Bytes32], program_input["storage_key_preimages"])
        ids.left = gen_arg(Optional[
            Union[InternalNode, Extended]
        ], program_input["pre_state_root"])
        ids.right = gen_arg(Optional[
            Union[InternalNode, Extended]
        ], program_input["post_state_root"])
    %}
    ...
    // DEBUG
    tempvar account_diff = account_diff;
    ...
    assert [output_ptr + 6] = state_account_diff_commitment;
    assert [output_ptr + 7] = state_storage_diff_commitment;
    assert [output_ptr + 8] = trie_account_diff_commitment;
    assert [output_ptr + 9] = trie_storage_diff_commitment;
