...
func set_storage{range_check_ptr, state: State}(
    ...
    // From EELS <https://github.com/ethereum/execution-specs/blob/master/src/ethereum/prague/state.py#L318>:
    // if trie._data == {}:
...
func set_transient_storage{range_check_ptr, transient_storage: TransientStorage}(
    ...
    // From EELS https://github.com/ethereum/execution-specs/blob/5c82ed6ac3eb992c7d87320a3e771b5e852a06df/src/ethereum/prague/state.py#L697:
    // if trie._data == {}:
...
func finalize_state{range_check_ptr, state: State}() {
    ...
    // The original_storage_tries mapping is transaction-specific, and as such, should have been squashed & set to 0 at this time.
    let original_storage_tries = state.value.original_storage_tries;
    with_attr error_message("InvariantError: original_storage_tries should be 0") {
        assert cast(original_storage_tries.value, felt) = 0;
    }
