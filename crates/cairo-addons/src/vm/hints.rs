...
/// Get the set of registered hint IDs
pub fn get_registered_hint_ids() -> std::collections::HashSet<String> {
    let mut hint_ids = std::collections::HashSet::new();

    // Add built-in hints
    hint_ids.insert(String::from("memory[ap] = to_felt_or_relocatable(segments.add())"));
    hint_ids.insert(String::from(SHA256_FINALIZE));

    // Add hints from modules
    for hint_fn in DICT_HINTS
        .iter()
        .chain(HASHDICT_HINTS.iter())
        .chain(UTILS_HINTS.iter())
        .chain(BYTES_HINTS.iter())
        .chain(MATHS_HINTS.iter())
        .chain(ETHEREUM_HINTS.iter())
        .chain(CURVE_HINTS.iter())
        .chain(CIRCUITS_HINTS.iter())
        .chain(PRECOMPILES_HINTS.iter())
    {
        let hint = hint_fn();
        hint_ids.insert(hint.id);
    }
