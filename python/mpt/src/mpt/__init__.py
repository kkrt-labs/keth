        ...
        if isinstance(node, BranchNode):
            ...
            if common_prefix_len < len(key_segment):
                ...
                if common_prefix_len + 1 < len(nibble_path):
                    ...
                    branch_node = BranchNode(
                        subnodes=tuple(branch_subnodes), value=value
                    )
            ...
            if nibble_path == leaf_key:
                ...
                return LeafNode(rest_of_key=leaf_key, value=value), True

            # Find the common prefix length
            common_prefix_len = 0
            for i in range(min(len(leaf_key), len(nibble_path))):
                if leaf_key[i] != nibble_path[i]:
                    break
                common_prefix_len += 1
            ...
            if common_prefix_len < len(leaf_key):
                # Create a new leaf node with the remaining key
                new_leaf = LeafNode(
                    rest_of_key=leaf_key[common_prefix_len + 1 :], value=node.value
                )
                ...
                if len(encoded_leaf) >= 32:
                    leaf_hash = keccak256(encoded_leaf)
                    self.nodes[leaf_hash] = encoded_leaf
                    branch_subnodes[leaf_key[common_prefix_len]] = leaf_hash
                else:
                    branch_subnodes[leaf_key[common_prefix_len]] = encoded_leaf
                branch_node = BranchNode(subnodes=tuple(branch_subnodes), value=b"")
            else:
                # The leaf ends at the branch, add its value
                branch_node = BranchNode(
                    subnodes=tuple(b"" for _ in range(16)), value=node.value
                )

            # Add the new path as another branch
            if common_prefix_len < len(nibble_path):
                # Create a new leaf node with the remaining path
                new_leaf = LeafNode(
                    rest_of_key=nibble_path[common_prefix_len + 1 :], value=value
                )
